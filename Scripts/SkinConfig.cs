...
namespace AntlerShed.SkinRegistry
{
    class SkinConfig
    {
        ...
        internal ConfigEntry<bool> DebugSetting { get; private set; }
        ...
        internal SkinConfig(ConfigFile config)
        {
            ...
            ConfigEntry<bool> entry = configFile.Bind("Debug", "DebugOn", false, "Flag for activating debug messages.");
        }

        private void SettingChanged(object sender, SettingChangedEventArgs e)
        {
            if (EnemySkinRegistry.Debug)
            {
                EnemySkinRegistry.SkinLogger.LogInfo("Settings change fired");
            }
        }
            ...
            if (!configInitialized)
            {
                foreach (EnemyInfo info in loadedEnemies)
                {
                    ...
                    try
                    {
                        ...
                        //All this is doing is filtering out moons and skins that are no longer installed
                        EnemyConfiguration sanitizedConfigEntry = new EnemyConfiguration
                        (
                            configEntry.EnemyId,
                            new MapConfiguration(configEntry.DefaultConfiguration.VanillaFrequency, configEntry.DefaultConfiguration.Distribution.Where((skin) => enemySkins.Any((lskin) => lskin.Id.Equals(skin.SkinId))).ToArray()),
                            ...
                            configEntry.ActiveSkins.Where((id) => loadedSkins.Any((skin) => skin.Id.Equals(id))).ToArray()
                        );
                ...
                foreach (KeyValuePair<ConfigDefinition, ConfigEntryBase> pair in configFile.Where((entry) => !loadedEnemies.Any((enemy) => enemy.Id.Equals(entry.Key.Key))))
                {
                    configFile.Remove(pair.Key);
                }
                configFile.Save();
        ...
        internal EnemyConfiguration? GetConfiguration(string enemyId)
        {
            ...
            {
                EnemySkinRegistry.SkinLogger.LogWarning($"Attempted to read config entry for {enemyId}, but no entry exists in the config");
            }
            return null;
        }
    }
